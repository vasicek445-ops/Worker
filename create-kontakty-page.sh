#!/bin/bash
# Create /kontakty page - agency search with filters
set -e
echo "📇 Creating Kontakty page..."

# 1. Create the API route to fetch agencies from Supabase
mkdir -p app/api/agencies
cat > app/api/agencies/route.ts << 'APIEND'
...
# 2. Create the Kontakty page
cat > app/kontakty/page.tsx << 'PAGEEND'
'use client'
...
echo "✅ Kontakty page created!"
echo ""
echo "Files created:"
echo "  📁 app/api/agencies/route.ts (API endpoint)"
echo "  📁 app/kontakty/page.tsx (page with search + filters)"
echo ""
echo "Run:"
echo "  git add -A && git commit -m 'feat: add kontakty page with agency search' && git push"
