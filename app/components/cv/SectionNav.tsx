...
// Levý sidebar 64px na desktopu, bottom-tabs na mobilu.
        ...
        {SECTIONS.map((s) => {
          ...
          return (
            <button
              ...
              title={s.label}
              className={`relative mx-2 h-12 rounded-xl flex items-center justify-center text-xl transition-all group ${
                isActive
                  ? 'bg-[#fb923c]/15 border border-[#fb923c]/40'
                  : 'hover:bg-white/[0.04]'
              }`}
            >
              <span>{s.icon}</span>
              ...
              <span className="absolute left-full ml-3 px-2 py-1 rounded-md bg-[#1f1f2e] text-xs whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-50">
                {s.label}
              </span>
            </button>
          )
