...
// Extract roky z textu (např. "2019-2024" → "5", "5 let praxe" → "5")
function extractYearsFromText(text: string): string {
  if (!text) return ''
  const range = text.match(/(\d{4})\s*[-–]\s*(\d{4})/)
  if (range) {
    const start = parseInt(range[1], 10)
    const end = parseInt(range[2], 10)
    if (end > start && end - start < 60) return String(end - start)
  }
...
export default function MotivacniDopis() {
  ...
  useEffect(() => {
    ...
    setFormData((f) => {
      ...
      const apply = (key: string, val: string | null | undefined) => {
        if (!val) return
        if (next[key]?.trim()) return // user už něco napsal — nepřepisuj
        next[key] = val
        fromProfile.add(key)
      }
      apply('name', profile.full_name)
      apply('address', profile.adresa)
      apply('phone', profile.telefon)
      apply('email', profile.email)
      apply('position', profile.pozice)
      apply('field', profile.obor)
      apply('german', profile.nemcina_uroven)
      apply('skills', profile.dovednosti)
      // experience: pokus o roky z textu
      if (!next.experience?.trim() && profile.zkusenosti) {
        const years = extractYearsFromText(profile.zkusenosti)
        if (years) {
          next.experience = years
  ...
  // Job analysis prefill (sessionStorage / URL param)
  useEffect(() => {
    if (prefilled) return
  ...
  const handleChange = (name: string, value: string) => {
    ...
    // Pokud user změnil pole, badge "z profilu" zmizí
    setPrefilledFromProfile(prev => {
  ...
  const handleSaveLetter = async (html: string, pdfBlob: Blob) => {
    setSaving(true)
    try {
    ...
    return (
      <main className="min-h-screen bg-[#0a0a12] px-4 py-6 pb-24 relative overflow-hidden" style={{ fontFamily: "'Plus Jakarta Sans', -apple-system, sans-serif" }}>
        ...
        <div className="max-w-4xl mx-auto relative z-10">
              ...
              <div className="flex gap-1.5 flex-wrap">
                {LETTER_TEMPLATES.map((t) => (
                  <button key={t.id} onClick={() => setTemplate(t.id)}
                    className={`px-2.5 py-1.5 rounded-lg text-xs font-medium transition ${template === t.id ? 'bg-[#fb923c]/15 text-[#fb923c] border border-[#fb923c]/30' : 'bg-white/[0.03] text-white/40 hover:text-white/60 border border-white/[0.06]'}`}>
                    {t.icon} {t.name}
                  </button>
                ))}
              </div>
          ...
          {saved && (
            <div className="bg-[#fb923c]/[0.06] border border-[#fb923c]/20 rounded-xl p-3 mt-3 text-center">
              <span className="text-[#fb923c]/80 text-sm">Dopis uložen — bude se automaticky přikládat k přihláškám přes Smart Matching</span>
            </div>
          )}
        ...
        <div className="flex items-center gap-2 mb-6">
          {[1, 2, 3, 4].map(s => (
            <button key={s} onClick={() => setStep(s)}
              className={`flex-1 h-1.5 rounded-full transition-all ${s <= step ? 'bg-[#fb923c]/60' : 'bg-white/[0.06]'}`} />
          ))}
        </div>
            ...
            {step === 1 && (
              <>
                ...
                <div className="bg-[#111120]/80 backdrop-blur-sm rounded-2xl border border-white/[0.06] p-5">
                  ...
                  <div className="flex gap-1.5 mb-3 flex-wrap">
                    {categories.map((cat) => (
                      <button key={cat} onClick={() => setCategory(cat)}
                        className={`px-3 py-1.5 rounded-xl text-[10px] font-semibold transition-all ${category === cat ? 'bg-[#fb923c]/15 text-[#fb923c] border border-[#fb923c]/30 shadow-[0_0_10px_rgba(251,146,60,0.1)]' : 'bg-white/[0.03] text-white/30 hover:text-white/50 border border-white/[0.06]'}`}>
                        {cat}
                      </button>
                    ))}
                  </div>
                  ...
                  <div className="grid grid-cols-3 sm:grid-cols-4 gap-1.5 max-h-[280px] overflow-y-auto pr-1" style={{ scrollbarWidth: 'thin', scrollbarColor: 'rgba(255,255,255,0.1) transparent' }}>
                    {LETTER_TEMPLATES.filter(t => (t.category || 'Ostatní') === category).map((t) => (
                      <button key={t.id} onClick={() => setTemplate(t.id)}
                        className={`p-3 rounded-xl border text-center transition-all ${template === t.id ? 'border-[#fb923c]/40 bg-[#fb923c]/[0.08] shadow-[0_0_15px_rgba(251,146,60,0.08)]' : 'border-white/[0.06] bg-white/[0.02] hover:bg-white/[0.05] hover:border-white/[0.12]'}`}>
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    {QUICK_COLORS.map((c) => (
                      <button key={c} onClick={() => setAccentColor(c)}
                        ...
                        style={{ backgroundColor: c, border: accentColor === c ? '2.5px solid #fff' : '2.5px solid transparent', boxShadow: accentColor === c ? '0 0 0 2px rgba(255,255,255,0.3), 0 0 15px rgba(251,146,60,0.15)' : 'none' }} />
                    ))}
                  </div>
                  <div className="flex items-center gap-3">
                    <input
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          className="bg-white/[0.03] border border-white/[0.08] rounded-lg px-2.5 py-1 text-white text-xs font-mono w-20 focus:border-[#fb923c]/40 focus:outline-none"
                          maxLength={7}
                        />
                ...
                <button onClick={() => setStep(2)}
                  className="w-full bg-[#fb923c]/10 border border-[#fb923c]/20 text-[#fb923c] font-bold py-3.5 rounded-2xl transition hover:bg-[#fb923c]/20 hover:border-[#fb923c]/30">
              ...
              <>
                <div className="bg-[#111120]/80 backdrop-blur-sm rounded-2xl border border-white/[0.06] p-5 space-y-3">
                  ...
                  <div className="flex items-center gap-4 mb-2">
                    <label className="relative cursor-pointer group">
                      <div className={`w-16 h-16 rounded-full border-2 border-dashed flex items-center justify-center overflow-hidden transition ${photo ? 'border-[#fb923c]/40' : 'border-white/15 hover:border-white/30'}`}>
                        {photo ? (
                  ...
                  <div className="relative">
                    ...
                    {prefilledFromProfile.has('address') && (
                      <span className="absolute top-1 right-2 text-[9px] text-[#fb923c]/70 bg-[#fb923c]/[0.08] border border-[#fb923c]/20 rounded-md px-1.5 py-0.5">z profilu</span>
                    )}
                  </div>
                ...
                <div className="flex gap-3">
                  ...
                  <button onClick={() => setStep(3)}
                    className="flex-[2] bg-[#fb923c]/10 border border-[#fb923c]/20 text-[#fb923c] font-bold py-3.5 rounded-2xl transition hover:bg-[#fb923c]/20 hover:border-[#fb923c]/30">
                    Pokračovat →
                  </button>
                </div>
              </>
              ...
              <>
                <div className="bg-[#111120]/80 backdrop-blur-sm rounded-2xl border border-white/[0.06] p-5 space-y-3">
                  ...
                  <div className="relative">
                    ...
                    {prefilledFromProfile.has('skills') && (
                      <span className="absolute top-1 right-2 text-[9px] text-[#fb923c]/70 bg-[#fb923c]/[0.08] border border-[#fb923c]/20 rounded-md px-1.5 py-0.5">z profilu</span>
                    )}
                  </div>
                ...
                <div className="flex gap-3">
                  ...
                  <button onClick={handleSubmit} disabled={generating}
                    className="flex-[2] relative overflow-hidden bg-gradient-to-r from-[#fb923c] to-[#f97316] text-[#0a0a12] font-extrabold py-3.5 px-6 rounded-2xl transition-all disabled:opacity-30 disabled:cursor-not-allowed hover:shadow-[0_4px_30px_rgba(251,146,60,0.35)] hover:scale-[1.02] active:scale-[0.98]">
                    {generating ? (
